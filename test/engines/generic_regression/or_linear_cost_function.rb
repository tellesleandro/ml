...
class ORLinearCostFunctionTest < ActiveSupport::TestCase
  ...
  test "should minimize errors" do
    ...
    assert_in_delta(
      cost_function_minimized.subs({@x1 => 0, @x2 => 0}), 0.25, 0.01
    )
    assert_in_delta(
      cost_function_minimized.subs({@x1 => 0, @x2 => 1}), 0.75, 0.01
    )
    assert_in_delta(
      cost_function_minimized.subs({@x1 => 1, @x2 => 0}), 0.75, 0.01
    )
    assert_in_delta(
      cost_function_minimized.subs({@x1 => 1, @x2 => 1}), 1.25, 0.01
    )
  end
